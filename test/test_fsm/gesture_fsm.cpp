    ...
    switch (prev) {
      case IDLE:
        if (g == DRAG_GESTURE) {
          ...
          if (ax >=  3.0f) r.action = ACTION_SCROLL_UP,   r.nextState = SCROLL,   r.mouseEnabled = false;
          else if (ax <= -2.0f) r.action = ACTION_SCROLL_DOWN, r.nextState = SCROLL, r.mouseEnabled = false;
        }
        else if (g == ZOOM_GESTURE) {
          if      (ax >  3.0f) r.action = ACTION_ZOOM_IN;
          else if (ax <= -2.0f) r.action = ACTION_ZOOM_OUT;
